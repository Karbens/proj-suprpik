...
function contestDisplaySurvivor($id)
{
	...
	$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=pick">'.$makePicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=status">STATUS</a> | <a href="'.$mosConfig_live_site.'&tab=survivors">SURVIVORS</a></p>';
	...
	if(isset($_REQUEST['tab']) && ( $_REQUEST['tab'] == 'mypicks' || $_REQUEST['tab'] == 'pick' || $_REQUEST['tab'] == 'status'  || $_REQUEST['tab'] == 'survivors' ) )
	{
		...
		{
			$data = $contest_image.'<p> '.$myPicksConfig.' | <a href="'.$mosConfig_live_site.'&tab=pick">'.$makePicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=status">STATUS</a> | <a href="'.$mosConfig_live_site.'&tab=survivors">SURVIVORS</a></p>';
		...
		if($_REQUEST['tab'] == 'pick')
		{
			...
			{
				header('Location: '.$mosConfig_live_site.'?option=com_contests&contest_id='.$id.'&tab=status');
				exit();
			}
			
			$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | '.$makePicksConfig.' | <a href="'.$mosConfig_live_site.'&tab=status">STATUS</a> | <a href="'.$mosConfig_live_site.'&tab=survivors">SURVIVORS</a></p>';
		...
		if($_REQUEST['tab'] == 'status')
		{
			...
			$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=pick">'.$makePicksConfig.'</a> | STATUS | <a href="'.$mosConfig_live_site.'&tab=survivors">SURVIVORS</a></p>';
		...
		if($_REQUEST['tab'] == 'survivors')
		{
			...
			$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=pick">'.$makePicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=status">STATUS</a> | SURVIVORS</p>';
...
function contestDisplaySpread($id)
{
	...
	$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=pick">'.$makePicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=leaderboard">LEADERBOARD</a></p>';
	...
	if(isset($_REQUEST['tab']) && ( $_REQUEST['tab'] == 'mypicks' || $_REQUEST['tab'] == 'pick' || $_REQUEST['tab'] == 'status' || $_REQUEST['tab'] == 'leaderboard' ) )
	{
		if($_REQUEST['tab'] == 'mypicks')
		{
			...
			}else
			{
				if( $_REQUEST['user'] > 0 )
				{
					$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | ';
					$your_picks_title = 'Picks by '.$_REQUEST['username'];
				}else
				{
					$data = $contest_image.'<p> '.$myPicksConfig.' | ';
					$your_picks_title = 'Your Picks';
				}
				$data .= '<a href="'.$mosConfig_live_site.'&tab=pick">'.$makePicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=leaderboard">LEADERBOARD</a></p>';
				$picks = getContestUserPicks($id);//get the weekly survivor picks, max(21), 17 reg season and 4 playoffs
				...
				if(count($picks) > 0)
				{
					...
					foreach($picks as $pick)
					{
						...
						$pl += $pick_pei[1];
						$pt += $pick_pei[2];
						$p_points = ($pick_pei[0]*2) + $pick_pei[2];
						$tot_points += $p_points;
						...
						if( $_REQUEST['user'] > 0 && $_REQUEST['user'] != $user->ID)
						{
							if($pick['week_num'] < $curr_week)
							{
							  $data .= '<tr>
										<td align="center">'.$pick['week_num'].'</td>
										<td align="left">'.str_replace(',',', ',$pick['user_picks']).'</td>
		...
		if($_REQUEST['tab'] == 'pick')
		{
			$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | '.$makePicksConfig.' | <a href="'.$mosConfig_live_site.'&tab=leaderboard">LEADERBOARD</a></p>';
			...
			}else
			{
				...
				elseif( count($picks) == 0 )
				{
					$data .= '<p>Contest Odds are posted every Wednesday @ 3pm</p>';
				}
		...
		{
			$data = $contest_image.'<p> <a href="'.$mosConfig_live_site.'&tab=mypicks">'.$myPicksConfig.'</a> | <a href="'.$mosConfig_live_site.'&tab=pick">'.$makePicksConfig.'</a> | LEADERBOARD</p>';
...
function getCurrentExpiry($id=0)
{
	...
	if($id == 2)
	{
		$dtime = strtotime('2013-09-05') + (604800*$diff);
...
{
	//setcookie("contest[$id]" ,'true', mktime (0, 0, 0, 9, 1, 2014));
}
...
function getCurrentSurvivors($id)
{
	...
	$query  =  'SELECT u.ID as id, u.display_name as survivor, u.user_login as username, u.user_email as email, p.user_picks
				FROM br3_contests_picks p, br3_contests_nfl n, wp_users u
