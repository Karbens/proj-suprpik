...
class InitTeams extends Db_Sqlr_Basic implements ConstInt// stub function -- this will be replace with db functions
{
	...
	public function getAllInfoForCID($cid, $cemail)
	{
		$id = $cid;
		$query = mysql_query("SELECT * FROM `bracket_user` 
							  WHERE USERID = '".$id."' 
							  AND `EMAIL` = '".$cemail."'");
	...
	public function setAllInfoForCID($cid, $cemail, $answers)
	{
		$round = 0;
		
			//uncomment below to dissallow updating the bracket
			/*
			//check if user already has entry
			$info = $this->getAllInfoForCID($cid, $cemail);
			if ($info !== false)
			{
				return self::ROUND_ALREADY_SET;	
			}*/
			$expiry = strtotime('2012-03-15 12:00:00');
			$id = $cid;
			$em = $cemail;
			$cc = time();
			$pp = 1;
			...
			$sel_que = mysql_query("SELECT PRINTID FROM `bracket_user`
									WHERE USERID = '".$id."'
									AND EMAIL = '".$em."'
									LIMIT 1");
			
			if( @mysql_num_rows($sel_que) > 0)
			{
				$pres = mysql_fetch_row($sel_que);
				$query = mysql_query("UPDATE `bracket_user` 
									  SET
									   OPTIONS = '".$oo."', 
									   CREATED = '".$cc."', 
									   ROUND   = '".$rr."', 
									   ANSWERS = '".$an."'
									  WHERE PRINTID = ".$pres[0]);
			}
			else
			{
				$query = mysql_query("INSERT INTO `bracket_user` 
									 (PRINTID, OPTIONS, CREATED, ROUND, USERID, EMAIL, ANSWERS) 
									 VALUES 
									 (NULL,'".$oo."','".$cc."','".$rr."','".$id."', '".$em."', '".$an."')");
			}
			if ($query) {
	...
	protected function getTeam($id)
	{
		foreach ($this->teams as $t) {
			if ($t[0] == $id){	return $t;	}}	return false;
	}
	...
	public function setRoundPositionsOutput()
	{
		$groups = self::LINE_COUNT;
		$class = 'checkit';
		$s = array();
			...
			for ($j=0;$j<self::LINE_COUNT*2;$j++)
			{
				if ($j == $start)
				{
					if ($i == 1 || $i== 11)
					{
						if ($i==1)
						{
							$s[$i][$j] = '<td><input type="text" readonly="readonly" value="('.$lists[$current][$id][2].') '.$lists[$current][$id][1].'" id="team-'.$i.'-'.$lists[$current][$id][0].'"/>&nbsp;&nbsp;&nbsp;&nbsp;</td>';
						}
						else
						{
							$s[$i][$j] = '<td>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" readonly="readonly" value="('.$lists[$current][$id][2].') '.$lists[$current][$id][1].'" id="team-'.$i.'-'.$lists[$current][$id][0].'"/></td>';
					...
					else
					{
						if ($i==6 ) {
							...
							$s[$i][$j] = '<td><ins><input id="text-'.$i.'-'.$lists[$current][0].'-1" type="text"  value="'.$lists[$current][$id][1].'" readonly="readonly" class="disabled" ><input type="radio" style="visibility:hidden" name="final-'.$listing.'-'.$lists[$current][0].'" class="'.$class.'" value="'.$lists[$current][$id][0].' " selected="false" id="rt-'.$i.'-'.$lists[$current][0].'-1"/></ins><br /><ins><input type="text"  value="'.$lists[$current][$id][1].'" readonly="readonly" class="disabled" id="text-'.$i.'-'.$lists[$current][0].'-2"><input type="radio" style="visibility:hidden" name="final-'.$listing.'-'.$lists[$current][0].'" class="'.$class.'" value="'.$lists[$current][$id][0].' " selected="false" id="rt-'.$i.'-'.$lists[$current][0].'-2"/></ins></td>';	
							$start += $step;
						}
						else
						{
							...
							{
								$s[$i][$j] = '<td>
											  <input type="text" readonly="readonly"  value="'.$lists[$current][$id][1].'" class="disabled" >
											  &nbsp;&nbsp;
											  </td>';	
							} else
							{
	...
	public function createRoundTeams($pop = 1)
	{
		if (!empty($this->matchups))
		{
			($pop == 2) ? $this->setRoundPositionsOutput() : $this->setRoundPositions();
		}
	}
}
