...
if($_GET['league'] && trim($_GET['league']) != "")
{
  ...
  if($league_sport == 'Football')$tomorrow = strtotime("+10 days");//weekly lines for football
  if($league_id > 0)
  {
  		$xml = '';
		$period = 0;
		...
		foreach($games as $gm)
		{
			if( ($gm[rot_visitor]>0 && $gm[rot_home]>0) || (trim($gm[team_visitor])!="" && trim($gm[team_home])!="") )
			{
				...
				if( trim($gm[visitor_score]) != "" || trim($gm[home_score]) != "" )
				{
					...
					$gmp_a = ($gmp[0] > 0) ? ordinal($gmp[0]) : $gmp[0];
				...
				$gxml .= '<GAME id="'.$gm[game_id].'" date="'.$game_date.'" time="'.$game_time.'" seconds="'.$gm[game_seconds].'">
				      <TEAM number="'.$gm[rot_visitor].'" name="'.htmlspecialchars($gm[team_visitor]).'"'.$vs.$v_pitcher.'>
				...
				$gxml .='
				      ...
				      <TEAM number="'.$gm[rot_home].'" name="'.htmlspecialchars($gm[team_home]).'"'.$hs.$h_pitcher.'>
