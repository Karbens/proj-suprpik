				...
				if($st_desc == 'American Football' || $st_desc == 'Basketball' || $st_desc == 'Ice Hockey NHL' || $st_desc == 'Baseball')
				{
					...
					foreach($region as $rg)
					{
						   ...
						   $rg['attr']['name'] == 'MLB')
						{
							...
							{
								$league_id = 0;
								if($st_desc == 'Baseball' && eregi('MLB',$gp['attr']['name']) && eregi('Game Lines',$gp['attr']['name']) )$league_id = 1;
								...
								if($st_desc == 'Basketball' && eregi('NCAA',$gp['attr']['name']) && eregi('Game Lines',$gp['attr']['name']) )$league_id = 3;
		...
		*/
		...
		if($array[sbk] && count($stype) > 0)
		{
			...
			foreach($stype as $st)
			{
				...
				/*
				if( eregi("football_lines.xml",$this->feed) )
				{
					switch($st_id)
					{
						//NFL pre-season
				...
				if( eregi("all-liveodds.xml",$this->feed) )
				{
					switch($st_id)
					{
				...
				else
				{
					...
					if(eregi("MLB",$st_desc))$league_id = 1;
					if(eregi("NBA",$st_desc))$league_id = 2;
					if(eregi("NCAA Bk",$st_desc))$league_id = 3;
					if(eregi("NFL",$st_desc))$league_id = 4;
					if(eregi("NCAA Football",$st_desc))$league_id = 7;
					if(eregi("NHL",$st_desc))$league_id = 8;
				}
		...
		if($array[rss][channel] && count($array[rss][channel][item]) > 0)
		{
			...
			$lines_array = array();
			 ...
			 foreach($item as $it)
			 {
				...
				}//end of if(eregi(" @ ",$it[title][value]))
