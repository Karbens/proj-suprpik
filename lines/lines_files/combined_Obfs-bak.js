...
function Line(book_id, value, seconds) {
                            ...
                            if (team.nodeName == 'TEAM') {
                                ...
                                for (l = 0; l < lines.length; l++) {
                                    if (lines[l].nodeName == 'OPENER') {
                                        ...
                                        line_values[book + 1] = new Line(book_id, lines[l].getAttribute('value'), parseInt(seconds));
                                    }
                                }
                                if (number == '') {
...
function toggle_game_gone() {
    ...
    for (r = 0; r < total_rows; r++) {
        ...
        if (item.type == GAME) {
            if (show) {
                if (expand == '(-)') all_rows[r].style.display = "";
            } else {
                game = item.value;
...
function f(full_schedule, team_number, book, value, seconds) {
    ...
    if (full_schedule) P(game.bgcolor, value, color);
        ...
        cell = J(game.bgcolor, color, value);
                        ...
                        if (team.nodeName == 'TEAM') {
                            ...
                            for (l = 0; l < lines.length; l++) {
                                if (lines[l].nodeName == 'OPENER') {
                                    ...
                                    line_values[book + 1] = new Line(book_id, lines[l].getAttribute('value'), seconds);
...
function F(book, number, sport, game_date) {
    url = line_history_url + '&book=' + book + '&game=' + number + '&period=' + page_period + '&sport=' + sport + '&date=' + game_date.substring(0, 4) + '-' + game_date.substring(4, 6) + '-' + game_date.substring(6);
...
function L(bgcolor) {
    ...
    for (i = 0; i < number_of_books; i++) {
        ...
        d('<td style="cursor:pointer" class="' + game_col.type + '" onmouseover="change_fclass(this,\'on\');" onmouseout="change_fclass(this,\'off\');" onClick="F(' + books[i].id + ',\'' + display_team + '\',\'' + lg_sport + '\', \'' + game_date + '\')">' + game_col.value + '</td>');
    }
	
	game_col = new g(display_class + '-left', value);
    game_row[3] = game_col;
    d('<td id="s' + game_number + '" class="' + game_col.type + '">' + game_col.value + '</td>');
...
function P(bgcolor, value, color) {
    ...
    display_line[book] += J(bgcolor, color, value);
    if (home) display_line[book] += '</td></tr></table>';
    book++;
};

function J(bgcolor, color, value) {
    if (value.length == 0 || value == ' ') cell = '&nbsp;';
    else cell = value;
    cell += '&nbsp;&nbsp;';
    if (color == '1') return '<table width="100%" cellspacing="0" cellpadding="0"><tr><td class="' + changes_2_minutes + '-right">' + cell + '</td></tr></table>';
    else if (color == '2') return '<table width="100%" cellspacing="0" cellpadding="0"><tr><td class="' + changes_5_minutes + '-right">' + cell + '</td></tr></table>';
    else if (color == '3') return '<table width="100%" cellspacing="0" cellpadding="0"><tr><td class="' + changes_10_minutes + '-right">' + cell + '</td></tr></table>';
    else if (color == '9' && game_gone_color != 'whitebg') return '<table width="100%" cellspacing="0" cellpadding="0"><tr><td class="' + game_gone_color + '-right">' + cell + '</td></tr></table>';
    else if (bgcolor == 0) return '<table width="100%" cellspacing="0" cellpadding="0"><tr><td class="whitebg-right">' + cell + '</td></tr></table>';
    else
    return '<table width="100%" cellspacing="0" cellpadding="0"><tr><td class="greybg-right">' + cell + '</td></tr></table>';
};
