function processPromo() {
	...
	//check for events
	var cheCount = $('input:checkbox[name=eventchoice]:checked').length;
	if(cheCount < eventCount) {
		alert("Please select all the events.");
		return false;
	}
	...
	$('input:checkbox[name=eventchoice]:checked').each(function (index, domEle) {
   		// domEle == this
		if(ec > 0)entryValue += ',';
  ...
  function processPicks() {
	...
	//ajax the servData
	$.ajax({
	       url: "http://superpicks.com/contests/processPromo2.php",
	       type: "POST",
	       data: servData,
	       cache: false,
	       success: function (html) {
